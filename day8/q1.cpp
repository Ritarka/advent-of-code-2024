...
int antenna(std::vector<std::vector<char>>& input) {
    ...
    for (int i = 0; i < input.size(); i++) {
        for (int j = 0; j < input[0].size(); j++) {
            ...
            if (val == '.') continue;
            // if (locations.find(val) == locations.end()) locations.insert(vector<pair<int, int>>());
    ...
    for (const auto [antenna, coords] : locations) {
        for (int i = 0; i < coords.size(); i++) {
            ...
            for (int j = i+1; j < coords.size(); j++) {
                ...
                const auto xc = xa + distx;
                const auto yc = ya + disty;

                const auto xd = xb - distx;
                const auto yd = yb - disty;

                if (xc >= 0 && yc >= 0 && xc < input.size() && yc < input[0].size()) {
                    input[xc][yc] = '#';
                }
                if (xd >= 0 && yd >= 0 && xd < input.size() && yd < input[0].size()) {
                    // printf("[%d %d] [%d %d]\n", xc, yc, xd, yd);
                    input[xd][yd] = '#';
                }
